...
3. **Cut the first release** in the main `w2m` repo:
   ...
   `aarch64-apple-darwin` and `x86_64-unknown-linux-gnu`, and uploads them
   to the GitHub Release along with `.sha256` files. Intel macOS and ARM
   Linux are not currently published as prebuilts — `w2m.rb` directs those
   users to `cargo install --git` instead.
